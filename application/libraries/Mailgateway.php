...
class Mailgateway
{
    ...
    {
        $invoice_data           = json_decode($detail->invoice);
        $detail->invoice_id     = $invoice_data->invoice_id;
        $detail->sub_invoice_id = $invoice_data->sub_invoice_id;
        $detail->payment_id = $detail->invoice_id."/".$detail->sub_invoice_id;

        if ($detail->fee_category == "transport") {
            $fee = $this->_CI->studentfeemaster_model->getTransportFeeByInvoice($detail->invoice_id, $detail->sub_invoice_id);

        } else {

            $fee = $this->_CI->studentfeemaster_model->getFeeByInvoice($detail->invoice_id, $detail->sub_invoice_id);
        }
