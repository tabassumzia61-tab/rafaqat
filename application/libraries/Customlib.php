...
class Customlib
{
    ...
    public function getPaymenttype()
    {
        ...
        $payment_type['paid']    = $this->CI->lang->line('paid');
        return $payment_type;
    }
    ...
    public function getPageContentCategory(){
        ...
        $category['news']     = $this->CI->lang->line('news');
        $category['gallery']  = $this->CI->lang->line('gallery');
        return $category;
    }

    public function getSystemDateFormat($date_only = true, $time = false){
        $setting_result     = $this->CI->setting_model->get();
        $time_format = $setting_result[0]['time_format'];
        //print_r($setting_result[0]['time_format']);
        $hi_format = ' h:i';
        $Hi_format = ' H:i';
        if ($date_only && !$time) {
    ...
    {
        $setting_result = $this->CI->setting_model->getSystemDetail();
    ...
    public function getSystemCurrencyFormat()
    {
        $admin = $this->CI->session->userdata('admin');
        if ($admin) {
    ...
    public function getMonthDropdown($start_month = null){
        ...
        // echo $end_time = strtotime("+11 month", $start_time);
    ...
    public function getMonthNumber($month = null){
        $month_num = '';
        for ($i = 01; $i < 01 + 12; $i++) {
    ...
    public function getAppName()
    {
        $admin = $this->CI->session->userdata('admin');
        if ($admin) {
            return $admin['system_name'];
        } else if ($this->CI->session->userdata('student')) {
            $student = $this->CI->session->userdata('student');
            return $student['system_name'];
        }
    }
    ...
    public function get_searchtype()
    {
        ...
        $data = array(
            ...
            'period'        => $this->CI->lang->line('custom_period'),
        );
    ...
    public function get_postmessage()
    {
        ...
        if (isset($_POST['search_type']) && $_POST['search_type'] != '') {
            ...
            if ($_POST['search_type'] == "period") {
                ...
                $filter_record['search_type'] = $this->CI->lang->line('search_type') . ": " . $between_date['from_date'] . " " . $this->CI->lang->line('to') . " " . $between_date['to_date'];
            }
    ...
    public function generatestaffbarcode($employee_id)
    {
        //I'm just using rand() function for data example
        $data = [];
        $code = $employee_id;
