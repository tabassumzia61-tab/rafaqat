...
class Superadmin extends Public_Controller{
    ...
    public function __construct(){
        ...
        //$this->mailer;
        $this->system_setting = $this->setting_model->getSetting();
    }
                ...
                if ($result->is_active) {
                    if ($result->role_id == 1){
                        ...
                        $session_data = array(
                            ...
                            'system_name'            => $setting_result[0]['name'],
                        ...
                        $data['name'] = $app_name;
                        $data['error_message'] = 'Logged In Super Administrator';
    ...
    public function forgotpassword()
    {
        ...
        if ($this->form_validation->run() == false)
        {
            $this->load->view('admin/auth_forgot_password', $data);
        ...
        else
        {
            ...
            if ($result && $result->email != "")
            {
                if ($result->is_active == '1')
                {
                    ...
                    $name           = $result->name;
                    $resetPassLink  = site_url('user/reset-password.html') . "?v_code=" . $verification_code;
    ...
    {
        $verification_code  = $this->input->get('v_code');
        $app_name           = $this->setting_model->get();
        ...
        if ($user)
        {
            ...
            $this->form_validation->set_rules('current_password', $this->lang->line('current_password'), 'required|callback_check_current_password');
            $this->form_validation->set_rules('new_password', $this->lang->line('new_password'), 'required');
            $this->form_validation->set_rules('confirm_password', $this->lang->line('confirm_password'), 'required|matches[new_password]');
            
            if ($this->form_validation->run() == false)
            {    
                $data['verification_code'] = $verification_code;
                //render
                $this->load->view('admin/auth_reset_password', $data);
            }
            else
            {
                ...
                $update_record = array(
                    'id'                => $user->id,
                    'password'          => $this->enc_lib->passHashEnc($new_password),
                    'ch_password'        => $new_password,
    ...
    public function check_current_password($current_password)
    {
        ...
        if (!$user) {
            $this->form_validation->set_message('check_current_password', 'Invalid verification code.');
            return false;
        }

        // Compare hashed password
        if ($this->enc_lib->passHashDyc($current_password, $user->password)) {
            return true;
        } else {
