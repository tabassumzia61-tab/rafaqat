...
class Receiveitem extends Admin_Controller {
    ...
    function index() {
        if (!$this->rbac->hasPrivilege('receiveitem', 'can_add')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'receiveitem');
        $this->session->set_userdata('sub_menu', 'receiveitem/index');
        $data['title'] = 'receiveitem List';
        $itemcategory = $this->receiveitem_model->getProduct();
        ...
        $this->load->view('admin/receiveitem/receiveitemList', $data);
        $this->load->view('layout/footer', $data);
    }

    public function download($id) {
        $result = $this->receiveitem_model->getBillDetails($id);
        $this->media_storage->filedownload($result['attachment'], "./uploads/receiveitem");
    }
    ...
    function create(){
        ...
        $itemcategory = $this->receiveitem_model->getProduct();
        $data['itemcatlist'] = $itemcategory;
        $box = $this->receiveitem_model->getBox();
        $data['boxlist'] = $box;
        $supervisor = $this->staff_model->getStaffByRoleByBranch(4,1);
        $data['supervisor'] = $supervisor;
        $department   = $this->staff_model->getDepartment();
        ...
        $result = $this->receiveitem_model->getBillNo();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/receiveitem/receiveitemcreate', $data);
        $this->load->view('layout/footer', $data);
    ...
    function show_cart(){
        ...
        $box = $this->receiveitem_model->getBox();
        $data['boxlist'] = $box;
        $this->load->view('admin/receiveitem/cart/add_product_cart',$data);
    }
    ...
    function update_cart_item(){
        ...
        elseif ($type === 'box'){
            $data = array(
                'rowid' => $this->input->post('rowid'),
                'box_id'   => $this->input->post('o_val')
            );
        }
        elseif ($type === 'twe'){
            $data = array(
                'rowid' => $this->input->post('rowid'),
                'tweight'   => $this->input->post('o_val')
            );
        }
        elseif ($type === 'bwe'){
            $data = array(
                'rowid' => $this->input->post('rowid'),
                'bweight'   => $this->input->post('o_val')
            );
        }
        elseif ($type === 'nwe'){
            $data = array(
                'rowid' => $this->input->post('rowid'),
                'nweight'   => $this->input->post('o_val')
            );
        }
        $this->cart->update($data);
    }
    ...
    function save_receiveitem(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $img_name = $this->media_storage->fileupload("documents", "./uploads/receiveitem/");
            ...
            $this->db->insert('receiveitem', $data);
            $receiveitem_id = $this->db->insert_id();
            foreach ($this->cart->contents() as $item){
                $o_details['receiveitem_id']   = $receiveitem_id;
                ...
                $o_details['tweight']            = $item['tweight'];
                $o_details['bweight']            = $item['bweight'];
                $o_details['nweight']            = $item['nweight'];
                $o_details['box_id']            = $item['box_id'];
                $o_details['created_by']     = $userdata['id'];
                //save order details
                $this->db->insert('receiveitem_items', $o_details);
            }
    ...
    public function getBillDetails($id) {
        ...
        $result = $this->receiveitem_model->getBillDetails($id);
        $data['result'] = $result;
        $this->load->view('admin/receiveitem/printBill', $data);
    ...
    public function edit($id) {
        ...
        $s_data = array(
            ...
            'receiveitemid' => $result['id'],
        );
        $this->session->set_userdata($s_data);
        $this->load->view('layout/header');
        $this->load->view("admin/receiveitem/receiveitemedit", $data);
        $this->load->view('layout/footer');
    }

    function save_edit_receiveitem($id){
        $this->form_validation->set_rules('date', $this->lang->line('date'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('supervisor_id', $this->lang->line('supervisor'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('name', $this->lang->line('name'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('warehouse_id', $this->lang->line('warehouse'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['attachment'] = $img_name;
            if (isset($_FILES["documents"]) && $_FILES['documents']['name'] != '' && (!empty($_FILES['documents']['name']))) {
                $this->media_storage->filedelete($result['attachment'], "uploads/receiveitem");
            }
            $data['cart']               = json_encode($this->cart->contents());
            $this->db->where('receiveitem.id', $data['id']);
            $this->db->update('receiveitem', $data);
            ...
            if (!empty($add_result)) {
                foreach ($add_result as $add_key => $add_val) {
                    ...
                    foreach ($this->cart->contents() as $item){
                        if ($C_id === $item['id']) {
                            $o_details['receiveitem_id']   = $data['id'];
                            $o_details['warehouse_id']   = $this->input->post('warehouse_id');
                            $o_details['supervisor_id']  = $this->input->post('supervisor_id');
                            $o_details['dept_id']        = $this->input->post('dept_id');
                            $o_details['item_id']        = $item['id'];
                ...
                foreach ($pdetil_id as $key => $detailval){
                    ...
                    foreach ($this->cart->contents() as $item_key => $item_val) {
                        if ($C_id === $item_val['id']) {
                            ...
                            $o_details['box_id']            = $item['box_id'];
                            //save order details
                            $this->db->where('receiveitem_items.id', $o_details['id']);
                            $this->db->update('receiveitem_items', $o_details);
                        }
                    }
                }
            }
            if (!empty($delete_result)) {
                ...
                $this->receiveitem_model->remove_detial_product($d_delete_array);
    ...
    public function getreceiveitemlist(){
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                $deletebtn = '';
                $documents = '';
                if ($this->rbac->hasPrivilege('receiveitem', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/receiveitem/edit/" . $value->id . "'   data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i><span>".$this->lang->line('edit')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('receiveitem', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/receiveitem/deletereceiveitem/" . $value->id . "' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i><span>".$this->lang->line('delete')."</span></a>";
                }
                if ($value->attachment) {
                    $documents = "<a href='" . base_url() . "admin/receiveitem/download/" . $value->id . "' class='btn btn-warning btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'>
                ...
                $items = $this->receiveitem_model->getItemsByreceiveitemID($value->id);
                $itemname = '';
                $itemqty = '';
                $box_name = '';
                if(!empty($items)){
                    foreach($items as $ival){
                        $box_name_v = $this->receiveitem_model->getBoxNameByPID($ival['box_id']);
                        $itemname .= $ival['item_name'].'<br/>';
                        $box_name .= $box_name_v.'<br/>';
                        $itemqty .= $ival['qty'].'<br/>';
    ...
    public function getsearchreceiveitemlist(){
        $search_type = $this->input->post('search_type');
        $button_type = $this->input->post('button_type');
        $product_id  = $this->input->post('product_id');
        $supervisor_id = $this->input->post('supervisor_id');
        ...
        if ($button_type == 'search_filter') {
            ...
            $resultList        = $this->receiveitem_model->getreceiveitemByID($product_id,$supervisor_id,$dept_id,$shift, $date_from, $date_to);
        }
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                $items = $this->receiveitem_model->getItemsByreceiveitemIDByPID($value->id,$product_id);
                $itemqty = '';
                $itemname = '';
                $box_name = '';
                if(!empty($items)){
