...
class Staff extends Admin_Controller
{
    ...
    public function __construct()
    {
       ...
       // $this->load->library('mailsmsconf');
        ...
        $this->shift              = $this->config->item('shift');
    ...
    public function index(){
        ...
        if (isset($search)) {
            if ($search == 'search_filter') {
                ...
                $resultlist          = $this->staff_model->searchFullText($search_text,$selected_category, 1);
                $data['resultlist']  = $resultlist;
    ...
    public function disablestafflist()
    {
        ...
        if (isset($search)) {
            if ($search == 'search_filter') {
                ...
                $data['searchby']    = "text";
                $selected_value_staff_dis = $this->input->post('selected_value_staff_dis');
                $text_staff_dis = trim($this->input->post('text_staff_dis'));
                $data['text_staff_dis'] = $text_staff_dis;
                $data['selected_value_staff_dis'] = $selected_value_staff_dis;
                $resultlist          = $this->staff_model->searchFullText($text_staff_dis,$selected_value_staff_dis, 0);
                $data['resultlist']  = $resultlist;
                $data['title']       = 'Search Details: ' . $data['text_staff_dis'];
            }
        }
    ...
    public function create(){
        ...
        $data["paytypelist"]         = $this->accounts_model->getaccountsheadByID(7,1);
        $data['paydedtypelist']      = $this->accounts_model->getaccountsheadByID(8,1);
        $data["contract_type"]       = $this->contract_type;
        $data["shift_type"]           = $this->shift;
        $data['sch_setting']         = $this->sch_setting_detail;
        $data['staffid_auto_insert'] = $this->sch_setting_detail->staffid_auto_insert;
        $employee_id  = 0;
        if ($this->sch_setting_detail->staffid_auto_insert) {
            ...
            if (!empty($last_staff)) {
                $employees_id = $last_staff->employee_id + 1;
            }else{
                $employees_id = 1;
            }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $user_id   = $userlist["id"];
            $barch_id          = $this->input->post("branch_id");
            ...
            if ($this->input->post("date_of_joining") != "") {
                $date_of_joining   = $this->customlib->dateFormatToYYYYMMDD($this->input->post("date_of_joining"));
            }
            ...
            if ($insert) {
                ...
                $insert_id = $this->staff_model->batchInsert($data_insert, $leave_array, $data_setting);
                $staff_id  = $insert_id;
                $salary_type = $this->input->post("salary_type");
                $frequency = $this->input->post("frequency");
                $salary_amount = $this->input->post("salary_amount");
                if (!empty($salary_type)) {
                    foreach ($salary_type as $allwnckey => $allwncval) {
                        $amount = $salary_amount[$allwnckey];
                        $freq = $frequency[$allwnckey];
                        if($amount > 0){
                            $data_pay = array(
                                'staff_id' => $staff_id,
                                'frequency' => $freq,
                            ...
                            $this->staff_model->assignpaystaff($data_pay);
                        }
                    }
                }
                $salary_ded_type = $this->input->post("salary_ded_type");
                $salary_ded_amount = $this->input->post("salary_ded_amount");
                if (!empty($salary_ded_type)) {
                    foreach ($salary_ded_type as $allwncdedkey => $allwncdedval) {
                        $dedamount = $salary_ded_amount[$allwncdedkey];
                        if($dedamount > 0){
                            $data_pay = array(
                                'staff_id' => $staff_id,
                                'type_id' => $allwncdedval,
                                'brc_id' => $barch_id,
                                'amount' => $dedamount,
                                'created_by' => $user_id,
                                'created_at' => date('Y-m-d H:i:s'),
                            );
                            $this->staff_model->assignpaydedstaff($data_pay);
                        }
                    }
                }
                ...
                $data_doc = array('id' => $staff_id, 'resume' => $resume, 'joining_letter' => $joining_letter, 'resignation_letter' => $resignation_letter, 'other_document_file' => $fourth_doc);
    ...
    public function edit($id){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $salary_amount = $this->input->post("salary_amount");
            $paydif = [];
            if(!empty($payid) || !empty($paytypeid)){
                $paydif = array_diff($payid,$paytypeid);
            }
            if(!empty($paydif)){
                foreach($paydif as $delkey => $delval){
                    $this->staff_model->removepay($delval);
                }
            }
            if(!empty($salary_type)){
                foreach($salary_type as $typekey => $typeval){
                    $freq = $frequency[$typekey];
                    $amount = $salary_amount[$typekey];
                    if(!empty($paytypeid[$typekey])){
                        if($amount > 0){
                            $data_pay = array(
                                'id' => $paytypeid[$typekey],
                                'staff_id' => $id,
                                'frequency' => $freq,
                                'type_id' => $typeval,
                                'brc_id' => $branch_id,
            ...
            $paydedid = $this->input->post("paydedid");
            $paydedtypeid = $this->input->post("paydedtypeid");
            $salary_ded_type = $this->input->post("salary_ded_type");
            $salary_ded_amount = $this->input->post("salary_ded_amount");
            $paydeddif = [];
            if(!empty($paydedid) || !empty($paydedtypeid)){
                $paydeddif = array_diff($paydedid,$paydedtypeid);
            }
            if(!empty($paydeddif)){
                foreach($paydeddif as $deddelkey => $deddelval){
                    $this->staff_model->removepayded($deddelval);
                }
            }
            if(!empty($salary_ded_type)){
                foreach($salary_ded_type as $typededkey => $typededval){
                    $dedamount = $salary_ded_amount[$typededkey];
                    if(!empty($paydedtypeid[$typededkey])){
                        if($dedamount > 0){
                            $data_pay_ded = array(
                                'id' => $paydedtypeid[$typededkey],
                                'staff_id' => $id,
                                'type_id' => $typededval,
                                'brc_id' => $branch_id,
                                'amount' => $dedamount,
                            ...
                            $this->staff_model->assignpaydedstaff($data_pay_ded);
                        }
                    }else{
                        if($dedamount > 0){
                            $data_pay_ded = array(
    ...
    public function getEmployeeByRole()
    {
        ...
        $data = $this->staff_model->getEmployee($role,1,$brach_id);
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($id)) {
                $newdata = array(
                    ...
                    'ch_password' => $this->input->post('new_pass'),
                );
