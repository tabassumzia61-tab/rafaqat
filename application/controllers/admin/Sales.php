...
class Sales extends Admin_Controller{
    ...
    public function index($brnc_id = null){
        if (!$this->rbac->hasPrivilege('sales', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'sales');
        $this->session->set_userdata('sub_menu', 'sales/index');
        ...
        $this->load->view('admin/sales/salesList', $data);
    ...
    public function getsearchsaleslist(){
        $search_type = $this->input->post('search_type');
        $button_type = $this->input->post('button_type');
        $product_id  = $this->input->post('product_id');
        $customer_id = $this->input->post('customer_id');
        ...
        if ($button_type == 'search_filter') {
            ...
            $resultList        = $this->sales_model->getSales($brc_id,1,$product_id,$customer_id, $date_from, $date_to);
        }
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('sales', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/sales/edit/" . $value->id ."/" . $value->brc_id . "'   data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i><span>".$this->lang->line('edit')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('sales', 'can_delete')) {
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/sales/delete/" . $value->id . "/". $value->brc_id  ."' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i><span>".$this->lang->line('delete')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('sales', 'can_view')) {
                    $saleinvoicepdf = "<a href='" . base_url() . "report/pdfsaleinvoice/" . $value->id . "/". $value->brc_id  ."' title='" . $this->lang->line('sale_invoice') . "' data-toggle='tooltip'><i class='fa fa-download'></i><span>".$this->lang->line('sale_invoice')."</span></a>";
                }
                if ($value->attachment) {
                    $documents = "<a href='" . base_url() . "admin/sales/download/" . $value->id . "' class='btn btn-warning btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'>
                ...
                // $items = $this->sales_model->getItemsByQuotesByPID($value->id,$product_id);
                // $itemqty = '';
                // if(!empty($items)){
                ...
                // }
                 ...
                 $invoice_no = "<a href='#' data-toggle='modal' data-target='#salesinvoiceModal' data-sales_id='" . $value->id . "' data-toggle='tooltip'  title='".$this->lang->line('view_sale_invoice')."'>" . $value->sale_no . "</a>";
                $sts = '';
                if($value->status == 1){
    ...
    public function getsaleslist($brc_id){
        $m               = $this->sales_model->getSales($brc_id,1,"","","","");
        $m               = json_decode($m);
        $currency_symbol = $this->customlib->getSystemCurrencyFormat();
        $dt_data         = array();
        if (!empty($m->data)) {
    ...
    public function create($brc_id){
        if (!$this->rbac->hasPrivilege('sales', 'can_add')) {
            access_denied();
        }
        ...
        if ($sys_setting_detail->sale_auto_insert) {
            if ($sys_setting_detail->sale_update_status) {
                ...
                $last_sale = $this->sales_model->getsalesNo($brc_id);
                if(!empty($last_sale)){
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/salesCreate', $data);
        $this->load->view('layout/footer', $data);
    }
                ...
                if (isset($total_rows) && !empty($total_rows)) {
                    foreach ($total_rows as $row_key => $row_value) {
                        ...
                        if(!empty($product)){
                            ...
                            $o_details['sale_item_head_id']      = $product->acc_head_sales_id;
                            $o_details['sale_item_cost_head_id'] = $product->acc_head_cost_sale_id;
                            ...
                            //save order details
                            $this->db->insert('sale_items', $o_details);
                        }
                    }
                }
                if($this->input->post('customer_type') == 1){
    ...
    public function salesPrintSave(){
        $this->form_validation->set_rules('date', $this->lang->line('date'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('sale_no', $this->lang->line('sale_no'), 'trim|required|xss_clean');
        //$this->form_validation->set_rules('due_date', $this->lang->line('due').' '.$this->lang->line('date') , 'trim|required|xss_clean');
        if($this->input->post('customer_type') == 1){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if(!empty($sale_id)){
                ...
                $this->load->view('print/printsales', $data);
            }
            // $array = array('status' => 'success', 'error' => '', 'message' => 'Record Saved Successfully');
            // echo json_encode($array);
        }
    ...
    public function edit($id,$brc_id){
        ...
        $data['sales'] = $sales;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/salesEdit', $data);
        $this->load->view('layout/footer', $data);
    }

    function salesUpdateSave($id){
        $this->form_validation->set_rules('date', $this->lang->line('date'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('sale_no', $this->lang->line('sale_no'), 'trim|required|xss_clean');
        //$this->form_validation->set_rules('due_date', $this->lang->line('due').' '.$this->lang->line('date') , 'trim|required|xss_clean');
        if($this->input->post('customer_type') == 1){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->db->where('sales.id', $data['id']);
            ...
            if (!empty($delete_result)) {
                ...
                $this->sales_model->remove_detial_product($d_delete_array);
                ...
                foreach ($total_rows as $row_key => $row_value) {
                    ...
                    if(!empty($update_id)){
                        ...
                        if(!empty($product)){
                            ...
                            $o_details['subtotal']               = str_replace(",", "", $this->input->post('amount_'.$row_value));
                            $o_details['created_by']             = $userdata['id'];
                            $this->db->where('sale_items.id', $o_details['id']);
                            $this->db->update('sale_items', $o_details);
                        }
                    }else{
                        $item_id = $this->input->post('item_id_'.$row_value);
                        $product = $this->db->select('products.*,units.name as `unit_name`')->from('products')->join('units','units.id = products.unit','left')->where('products.id',$item_id)->get('')->row();
                        if(!empty($product)){
                            ...
                            $i_details['sale_item_head_id']      = $product->acc_head_sales_id;
                            $i_details['sale_item_cost_head_id'] = $product->acc_head_cost_sale_id;
                            ...
                            //save order details
                            $this->db->insert('sale_items', $i_details);
                        }
                    }
                }
            }
            if($this->input->post('customer_type') == 1){
                $sdet = $this->sales_model->getPaymentBySaleID($id);
                if(!empty($sdet)){
                    $userlist  = $this->customlib->getUserData();
                    $user_id   = $userlist["id"];
                    $data = array(
    ...
    public function delete($id,$brc_id){
        ...
        redirect('admin/sales/index/'.$brc_id);
    }
    ...
    public function getSalesInvoice($id) {
        ...
        $result = $this->sales_model->getSalesInvoiceByID($id);
        $data['result'] = $result;
        $this->load->view('admin/sales/printInvoice', $data);
        
    }
    
    public function salesreports(){
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreports');
        $this->session->set_userdata('subsub_menu', '');
        $this->load->view('layout/header');
        $this->load->view('admin/sales/reports/salesreports');
        $this->load->view('layout/footer');
    }
    
    function customerwisesalessum($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('customer_wise_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/sales/customerwisesalessum');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['salelist']               = $this->sales_model->getSalesByCustomersIDSummary($brc_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/reports/customerwisesalessum', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function customerwisesalesdet($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('customer_wise_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/sales/customerwisesalesdet');
        $data['title'] = 'Customer Wise Sales Details';
        if(!empty($brnc_id)){
            $brc_id = $brnc_id;
        }else{
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['end_date']               = $end_date;
            $data['customer_id']            = $customer_id;
            $data['salelist']               = $this->sales_model->getSalesByCustomersIDDetails($brc_id,$customer_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/reports/customerwisesalesdet', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function itemwisesalessum($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('customer_wise_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/sales/itemwisesalessum');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['end_date']               = $end_date;
            $data['customer_id']            = $customer_id;
            $data['salelist']               = $this->sales_model->getSalesByItemsIDSummary($brc_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/reports/itemwisesalessum', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function itemwisesalesdet($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('customer_wise_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/sales/salesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/sales/itemwisesalesdet');
        $data['title'] = 'Customer Wise Sales Details';
        if(!empty($brnc_id)){
            $brc_id = $brnc_id;
        }else{
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['salelist']               = $this->sales_model->getSalesByItemsIDDetails($brc_id,$item_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/sales/reports/itemwisesalesdet', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function viewsalesinvoice(){
        ...
        $data['inv'] = $this->sales_model->printsalesInsertID($sale_id);
        $this->load->view('admin/sales/salesinvoiceview', $data);
    }
