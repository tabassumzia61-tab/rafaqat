...
class Leaverequest extends Admin_Controller
{
    ...
    public function index(){
        ...
        $this->session->set_userdata('sub_menu', 'admin/leaverequest/index');
    ...
    public function applyleave(){
        ...
        $this->session->set_userdata('top_menu', 'attendance');
        $this->session->set_userdata('sub_menu', 'admin/staff/applyleave');
        $userdata                   = $this->customlib->getUserData();
    ...
    public function leaveRecord()
    {
        ...
        $result->leavefrom    = date($this->customlib->getSystemDateFormat(), $this->customlib->dateyyyymmddTodateformat($result->leave_from));
        $result->date         = date($this->customlib->getSystemDateFormat(), $this->customlib->dateyyyymmddTodateformat($result->date));
        $leave_to             = date("m/d/Y", strtotime($result->leave_to));
        $result->leaveto      = date($this->customlib->getSystemDateFormat(), $this->customlib->dateyyyymmddTodateformat($result->leave_to));
    ...
    public function addLeave()
    {
        ...
        $abranch_id   = $this->input->post("abranch_id");
        ...
        $this->form_validation->set_rules('abranch_id', $this->lang->line('branch'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $msg = array(
                'abranch_id'      => form_error('abranch_id'),
    ...
    public function add_staff_leave()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $leavefrom = date("Y-m-d", $this->customlib->datetostrtotime($this->input->post('leave_from_date')));
            $leaveto   = date("Y-m-d", $this->customlib->datetostrtotime($this->input->post('leave_to_date')));
            $staffdet     = $this->staff_model->get($userdata["id"]) ;
            $userbrach    = $staffdet['barch_id'];
            ...
            if ($total_remain >= $leave_days) {
                ...
                if (!empty($request_id)) {
                    ...
                    $data = array('staff_id' => $staff_id,'brach_id' => $userbrach, 'date' => date("Y-m-d", $this->customlib->datetostrtotime($applied_date)), 'leave_days' => $leave_days, 'leave_type_id' => $leavetype, 'leave_from' => $leavefrom, 'leave_to' => $leaveto, 'employee_remark' => $reason, 'status' => $status, 'admin_remark' => $remark, 'applied_by' => $applied_by, 'document_file' => $document);
                }
