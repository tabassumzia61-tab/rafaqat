...
class Purchases extends Admin_Controller{
    ...
    public function index($brnc_id = null){
        if (!$this->rbac->hasPrivilege('purchases', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'purchases');
        $this->session->set_userdata('sub_menu', 'purchases/index');
        ...
        $this->load->view('admin/purchases/purchasesList', $data);
    ...
    public function getsearchpurchaseslist(){
        $search_type = $this->input->post('search_type');
        $button_type = $this->input->post('button_type');
        $product_id  = $this->input->post('product_id');
        $supplier_id = $this->input->post('supplier_id');
        ...
        if ($button_type == 'search_filter') {
            ...
            $resultList        = $this->purchases_model->getpurchases($brc_id,1,$product_id,$supplier_id, $date_from, $date_to);
        }
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                $addrevdqty = '';
                $documents = '';
                if ($this->rbac->hasPrivilege('purchases', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/purchases/edit/" . $value->id . "/". $value->brc_id . "'   data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i><span>".$this->lang->line('edit')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('purchases', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/purchases/delete/" . $value->id . "/". $value->brc_id . "' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i><span>".$this->lang->line('delete')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('purchases', 'can_view')) {
                    $purchasebillpdf = "<a href='" . base_url() . "report/pdfpurchasesbill/" . $value->id . "/". $value->brc_id  ."' title='" . $this->lang->line('purchase_bill') . "' data-toggle='tooltip'><i class='fa fa-download'></i><span>".$this->lang->line('purchase_bill')."</span></a>";
                }
                if ($value->attachment) {
                    $documents = "<a href='" . base_url() . "admin/purchases/download/" . $value->id . "' class='btn btn-warning btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'>
                ...
                // $items = $this->purchases_model->getItemsByQuotesByPID($value->id,$product_id);
                // $itemqty = '';
                // if(!empty($items)){
                ...
                //     }
                // }
                $bill_no = "<a href='#' data-toggle='modal' data-target='#purchasebillModal' data-purchase_id='" . $value->id . "' data-toggle='tooltip'  title='".$this->lang->line('view_purchase_bill')."'>" . $value->purchase_no . "</a>";
                $sts = '';
                if($value->status == 1){
    ...
    {
        $result = $this->purchases_model->get($id);
        $this->media_storage->filedownload($result['attachment'], "./uploads/purchases");
    }

    public function getpurchaseslist($brc_id){
        $m               = $this->purchases_model->getpurchases($brc_id,1,"","","","");
        $m               = json_decode($m);
        $currency_symbol = $this->customlib->getSystemCurrencyFormat();
        $dt_data         = array();
        if (!empty($m->data)) {
    ...
    public function create($brc_id){
        if (!$this->rbac->hasPrivilege('purchases', 'can_add')) {
            access_denied();
        }
        ...
        if ($sys_setting_detail->purchase_auto_insert) {
            if ($sys_setting_detail->purchase_update_status) {
                ...
                $last_purchase = $this->purchases_model->getpurchasesNo($brc_id);
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/purchasescreate', $data);
        $this->load->view('layout/footer', $data);
    }

    public function purchasesSave(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($total_discount > 0){
                $discount_head_id = 18;
                $discount_pervious_head_id = 24;
            }else{
                $discount_head_id = NULL;
                $discount_pervious_head_id = NULL;
            }
            if($total_tax > 0){
                $gst_head_id = 1;
            }else{
            ...
            $data = [
                ...
                'note'                      => $note,
                'total'                     => $total,
                'discount_pervious_head_id' => $discount_pervious_head_id,
            ...
            if(!empty($purchase_id)){
                ...
                if (isset($total_rows) && !empty($total_rows)) {
                    foreach ($total_rows as $row_key => $row_value) {
                        ...
                        $o_details['purchase_item_head_id']      = $product->acc_head_purchases_id;
                        $o_details['purchase_cost_item_head_id'] = $product->acc_head_cost_sale_id;
                        $o_details['disc_pervious_head_id']      = $discount_pervious_head_id;
                        ...
                        //save order details
                        $this->db->insert('purchase_items', $o_details);
                    }
                }
                if($this->input->post('supplier_type') == 1){
    ...
    public function edit($id,$brc_id){
        ...
        $data['warehouseslist'] = $this->warehouses_model->get();
        $data["acclist"]        = $this->accounts_model->getaccountsheadByID(2,$brc_id);
        $purchases = $this->purchases_model->get($id);
        $data['purchases'] = $purchases;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/purchasesedit', $data);
        $this->load->view('layout/footer', $data);
    }

    function purchasesUpdateSave($id){
        $this->form_validation->set_rules('date', $this->lang->line('date'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('purchase_no', $this->lang->line('purchase_no'), 'trim|required|xss_clean');
        //$this->form_validation->set_rules('due_date', $this->lang->line('due').' '.$this->lang->line('date') , 'trim|required|xss_clean');
        if($this->input->post('supplier_type') == 1){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->db->where('purchases.id', $data['id']);
            ...
            if (!empty($delete_result)) {
                ...
                $this->purchases_model->remove_detial_product($d_delete_array);
            ...
            if (isset($total_rows) && !empty($total_rows)) {
                foreach ($total_rows as $row_key => $row_value) {
                    ...
                    if(!empty($update_id)){
                        ...
                        $o_details['subtotal']                   = str_replace(",", "",$this->input->post('amount_'.$row_value));
                        $o_details['created_by']                 = $userdata['id'];
                        $this->db->where('purchase_items.id', $o_details['id']);
                        $this->db->update('purchase_items', $o_details);
                    }else{
                        $item_id = $this->input->post('item_id_'.$row_value);
                        $product = $this->db->select('products.*,units.name as `unit_name`')->from('products')->join('units','units.id = products.unit','left')->where('products.id',$item_id)->get('')->row();
                        ...
                        $i_details['discount_head_id']           = $discount_head_id;
                        $i_details['disc_pervious_head_id']      = $discount_pervious_head_id;
                        $i_details['gst_head_id']                = $gst_head_id;
                        $i_details['purchase_item_head_id']      = $product->acc_head_purchases_id;
                        ...
                        //save order details
                        $this->db->insert('purchase_items', $i_details);
                    }
                }
            }
            if($this->input->post('supplier_type') == 1){
                $pdet = $this->purchases_model->getPaymentByPurchaseID($id);
                if(!empty($pdet)){
                    $userlist  = $this->customlib->getUserData();
                    $user_id   = $userlist["id"];
                    $data = array(
    ...
    public function getBillDetails($id) {
        ...
        $result = $this->purchases_model->getPurchaseBillByID($id);
        $data['result'] = $result;
        $this->load->view('admin/purchases/printBill', $data);
        
    }
    
    public function purchasesreports(){
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/purchases/purchasesreports');
        $this->session->set_userdata('subsub_menu', '');
        $this->load->view('layout/header');
        $this->load->view('admin/purchases/reports/purchasesreports');
        $this->load->view('layout/footer');
    }
    
    function supplierwisepurchasessum($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('supplier_wise_purchases_summary', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/purchases/purchasesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/purchases/supplierwisepurchasessum');
        $data['title'] = 'Supplier Wise Purchases Details';
        if(!empty($brnc_id)){
            $brc_id = $brnc_id;
        }else{
            $brc_id = $this->customlib->getBranchID();
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['purchaseslist']               = $this->purchases_model->getPurchasesBySuppliersIDSummary($brc_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/reports/supplierwisepurchasessum', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function supplierwisepurchasesdet($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('supplier_wise_purchases_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/purchases/purchasesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/purchases/supplierwisepurchasesdet');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['purchaseslist']          = $this->purchases_model->getPurchasesBySuppliersIDDetails($brc_id,$supplier_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/reports/supplierwisepurchasesdet', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function itemwisepurchasessum($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('produts_wise_purchases_summary', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/purchases/purchasesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/purchases/itemwisepurchasessum');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['start_date']             = $start_date;
            $data['end_date']               = $end_date;
            $data['purchaseslist']          = $this->purchases_model->getPurchasesByItemsIDSummary($brc_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/reports/itemwisepurchasessum', $data);
        $this->load->view('layout/footer', $data);
    }
    
    function itemwisepurchasesdet($brnc_id = null) {
        if (!$this->rbac->hasPrivilege('produts_wise_purchases_details', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/purchases/salesreports');
        $this->session->set_userdata('subsub_menu', 'Reports/purchases/itemwisepurchasesdet');
        $data['title'] = 'Customer Wise Sales Details';
        if(!empty($brnc_id)){
            $brc_id = $brnc_id;
        }else{
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['purchaseslist']          = $this->purchases_model->getPurchasesByItemsIDDetails($brc_id,$item_id,$start_date,$end_date);
        }
        $this->load->view('layout/header', $data);
        $this->load->view('admin/purchases/reports/itemwisepurchasesdet', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function viewpurchasebill(){
        ...
        $data['inv'] = $this->purchases_model->printpurchasesInsertID($purchase_id);
        $this->load->view('admin/purchases/purchasebillview', $data);
    }
