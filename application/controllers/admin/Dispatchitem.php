...
class Dispatchitem extends Admin_Controller {
    ...
    function index() {
        if (!$this->rbac->hasPrivilege('dispatchitem', 'can_add')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'dispatch');
        $this->session->set_userdata('sub_menu', 'dispatchitem/index');
        $data['title'] = 'dispatch item List';
        $itemcategory = $this->dispatchitem_model->getProduct();
        ...
        $this->load->view('admin/dispatchitem/dispatchitemList', $data);
        $this->load->view('layout/footer', $data);
    }

    public function download($id) {
        $result = $this->dispatchitem_model->getBillDetails($id);
        $this->media_storage->filedownload($result['attachment'], "./uploads/dispatchitem");
    }
    ...
    function create(){
        $this->session->set_userdata('top_menu', 'dispatchitem');
        $this->session->set_userdata('sub_menu', 'dispatchitem/index');
        $this->cart->destroy();
        $data = array(
            'tax' => 0,
        ...
        $result = $this->dispatchitem_model->getBillNo();
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('admin/dispatchitem/dispatchitemcreate', $data);
        $this->load->view('layout/footer', $data);
    ...
    function show_cart(){
        $itemcategory = $this->dispatchitem_model->getProduct();
        $data['itemcatlist'] = $itemcategory;
        $this->load->view('admin/dispatchitem/cart/add_product_cart',$data);
    }
    ...
    function save_dispatchitem(){
        ...
        if ($this->form_validation->run() == false) {
            ...
            $img_name = $this->media_storage->fileupload("documents", "./uploads/dispatchitem/");
            ...
            $this->db->insert('dispatchitem', $data);
            $dispatchitem_id = $this->db->insert_id();
            foreach ($this->cart->contents() as $item){
                $o_details['dispatchitem_id']   = $dispatchitem_id;
                ...
                //save order details
                $this->db->insert('dispatchitem_items', $o_details);
            }
    ...
    public function getBillDetails($id) {
        ...
        $result = $this->dispatchitem_model->getBillDetails($id);
        $data['result'] = $result;
        $this->load->view('admin/dispatchitem/printBill', $data);
    ...
    public function edit($id) {
        ...
        $s_data = array(
            ...
            'dispatchitemid' => $result['id'],
        );
        $this->session->set_userdata($s_data);
        $this->load->view('layout/header');
        $this->load->view("admin/dispatchitem/dispatchitemedit", $data);
        $this->load->view('layout/footer');
    }

    function save_edit_dispatchitem($id){
        $this->form_validation->set_rules('date', $this->lang->line('date'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('supervisor_id', $this->lang->line('supervisor'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('name', $this->lang->line('name'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('warehouse_id', $this->lang->line('warehouse'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['attachment'] = $img_name;
            if (isset($_FILES["documents"]) && $_FILES['documents']['name'] != '' && (!empty($_FILES['documents']['name']))) {
                $this->media_storage->filedelete($result['attachment'], "uploads/dispatchitem");
            }
            $data['cart']               = json_encode($this->cart->contents());
            $this->db->where('dispatchitem.id', $data['id']);
            $this->db->update('dispatchitem', $data);
            ...
            if (!empty($add_result)) {
                foreach ($add_result as $add_key => $add_val) {
                    ...
                    foreach ($this->cart->contents() as $item){
                        if ($C_id === $item['id']) {
                            $o_details['dispatchitem_id']   = $data['id'];
                            $o_details['warehouse_id']   = $this->input->post('warehouse_id');
                            $o_details['supervisor_id']  = $this->input->post('supervisor_id');
                            $o_details['dept_id']        = $this->input->post('dept_id');
                            $o_details['item_id']        = $item['id'];
                ...
                foreach ($pdetil_id as $key => $detailval){
                    ...
                    foreach ($this->cart->contents() as $item_key => $item_val) {
                        if ($C_id === $item_val['id']) {
                            ...
                            //save order details
                            $this->db->where('dispatchitem_items.id', $o_details['id']);
                            $this->db->update('dispatchitem_items', $o_details);
                        }
                    }
                }
            }
            if (!empty($delete_result)) {
                ...
                $this->dispatchitem_model->remove_detial_product($d_delete_array);
    ...
    public function deletedispatchitem($id) {
        if (!empty($id)) {
            $this->dispatchitem_model->deletedispatchitem($id);
            $this->session->set_flashdata('msg', '<div class="alert alert-danger text-left">Record Deleted successfully</div>');
            redirect('admin/dispatchitem/index/');
        }
    }
    
    public function getdispatchitemlist(){
        $m               = $this->dispatchitem_model->getdispatchitemAll();
        $m               = json_decode($m);
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                $deletebtn = '';
                $documents = '';
                if ($this->rbac->hasPrivilege('dispatchitem', 'can_edit')) {
                    $editbtn = "<a href='" . base_url() . "admin/dispatchitem/edit/" . $value->id . "'   data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i><span>".$this->lang->line('edit')."</span></a>";
                }
                if ($this->rbac->hasPrivilege('dispatchitem', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm(" . '"' . $this->lang->line('delete_confirm') . '"' . ");' href='" . base_url() . "admin/dispatchitem/deletedispatchitem/" . $value->id . "' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i><span>".$this->lang->line('delete')."</span></a>";
                }
                if ($value->attachment) {
                    $documents = "<a href='" . base_url() . "admin/dispatchitem/download/" . $value->id . "' class='btn btn-warning btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('download') . "'>
                ...
                $items = $this->dispatchitem_model->getItemsBydispatchitemID($value->id);
                $itemname = '';
                $itemqty = '';
                if(!empty($items)){
    ...
    public function searchdispatchitem(){
    ...
    public function getsearchdispatchitemlist(){
        $search_type = $this->input->post('search_type');
        $button_type = $this->input->post('button_type');
        $product_id  = $this->input->post('product_id');
        $supervisor_id = $this->input->post('supervisor_id');
        ...
        if ($button_type == 'search_filter') {
            ...
            $resultList        = $this->dispatchitem_model->getdispatchitemByID($product_id,$supervisor_id,$dept_id,$shift, $date_from, $date_to);
        }
        ...
        if (!empty($m->data)) {
            ...
            foreach ($m->data as $key => $value) {
                ...
                $items = $this->dispatchitem_model->getItemsBydispatchitemIDByPID($value->id,$product_id);
                $itemqty = '';
                $itemname = '';
                if(!empty($items)){
                    foreach($items as $ival){
