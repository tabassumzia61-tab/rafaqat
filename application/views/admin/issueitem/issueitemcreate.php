        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    <form id="submit_salebill" action="<?php echo site_url('admin/issueitem/save_issueitem') ?>" name="employeeform" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="tshadow mb25 bozero">    
                                <h4 class="pagetitleh2"><?php echo $this->lang->line('add_issueitem'); ?> </h4>
                                <div class="around10">
                                    ...
                                    <div id="cart_view" class="row" style="padding:7px 15px 0px 17px;">
                                       <?php echo $this->view('admin/issueitem/cart/add_product_cart','', TRUE); ?>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
...
<script type="text/javascript">
    function get_data_by_campuss(val){
        //alert();
        var url ='<?php echo site_url('admin/issueitem/newpurchase/'); ?>'+val;          
        if(url){
    ...
    $(document).ready(function () {
        $(document).on('click', '.submit_salebill', function (e) {
            ...
            if(itemsel == ''){
                ...
                setTimeout(function () {
                    $.ajax({
                        url: '<?php echo site_url("admin/issueitem/save_issueitem") ?>',
                        type: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
...
<script type="text/javascript">
    ...
    function pur_product_id(str) {
        ...
        //loader();
        $.ajax({
            url: '<?php echo site_url("admin/issueitem/add_to_cart") ?>',
            type: "POST",
            data: {
                item_id : itemID,
                rowid: rowid,
            ...
            success: function(response) {
                ...
                $.get(link + "admin/issueitem/show_cart/", function (cart) {
                    //alert(cart);
                    $('#cart_view').html(cart);
                    $('.js-example-basic-single').select2();
                    //$('select').select2();
    ...
    function pur_updateItem(str) {
        ...
        var o_val = str.value;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/issueitem/update_cart_item';
        //var csrftoken   = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_removeItem(str) {
        var rowid = str.id;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/issueitem/remove_item';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_order_discount(str) {
        ...
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/issueitem/order_discount';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_tax(str) {
        var tax =  str.value;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/issueitem/order_tax';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_shipping(str) {
        ...
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/issueitem/order_shipping';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function get_vendor(str) {
        ...
        var postUrl     = getBaseURL() + 'admin/issueitem/select_vendor_by_id';
        var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function getPurchaseProduct(str){
        ...
        if (req) {
            var link = '<?php echo base_url() ?>';
            $.post(link + "admin/issueitem/add_purchase_product", {
                    purchase_product_id: purchaseProductId,
                    ajax: '1'
                },
                function (data) {
                    if (data == 'true') {
                        $.get(link + "admin/issueitem/show_purchase", function (cart) {
