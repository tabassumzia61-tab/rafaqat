<div class="col-md-12">
    <form id="submitrecvdqty" action="<?php echo site_url('admin/purchases/addproductsrecvd') ?>" name="employeeform" method="post" accept-charset="utf-8" enctype="multipart/form-data">
        <div class="row">  
                    ...
                    <tbody>
                        <?php 
                            ...
                            foreach ($cartItem as $option) { 
                                $detpr = $this->purchases_model->getAllPurchaseItemsRecvd($inv['id'],$option->id);
                                if(!empty($detpr)){
                                    $recvd_qty = 0;
                                    foreach($detpr as $det_val){
                                        $recvd_qty = $recvd_qty + $det_val->quantity_received;
                                    }
                                $bal = $option->qty - $recvd_qty;
                                if($bal > 0){
                                ?>
                                <tr>
                                    <td>
                                    ...
                                    <td><input class="form-control text-center rquantity" name="quantity[]" type="number" value="<?php  echo (float)$bal; ?>" /></td>                                
                                </tr>
                            <?php } ?>
                        <?php } else { ?> 
                            <tr>
                                ...
                                <td><input class="form-control text-center rquantity" name="quantity[]" type="number" value="<?= (float)$option->qty; ?>" /></td>                                
                            </tr>
                        <?php } ?>
                        <?php } ?>
                    </tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.submitrecvdqty', function (e) {
            ...
            $this.button('loading');
            $.ajax({
                url: '<?php echo site_url("admin/purchases/addproductsrecvd") ?>',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
