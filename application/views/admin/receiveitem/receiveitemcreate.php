        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    <form id="submit_salebill" action="<?php echo site_url('admin/receiveitem/save_receiveitem') ?>" name="employeeform" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="tshadow mb25 bozero">    
                                <h4 class="pagetitleh2"><?php echo $this->lang->line('add_receiveitem'); ?> </h4>
                                <div class="around10">
                                    ...
                                    <div class="row" style="padding:7px 15px 0px 17px;">
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                ...
                                                <input name="date" id="datetime_twelve_hour"  type="text" value="<?php echo date($this->customlib->getSystemDateFormat(true,true)); ?>" class="form-control datetime_twelve_hour"/>
                                        ...
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                ...
                                                <small class="req" style="color:red;"> *</small> 
                                                <input name="reference_no" id="reference_no" type="text" class="form-control" value="<?php echo $bill_no; ?>" readonly="readonly"/>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select  id="supervisor_id" name="supervisor_id" class="form-control js-example-basic-single"  >
                                                    ...
                                                    foreach ($supervisor as $val) {
                                                        ?>
                                                        <option value="<?php echo $val['id'] ?>" <?php if (set_value('supervisor_id') == $val['id']) echo "selected=selected" ?>><?php echo $val['name'] ?></option>
                                                        <?php
                                    ...
                                    <div id="cart_view" class="row" style="padding:7px 15px 0px 17px;">
                                       <?php echo $this->view('admin/receiveitem/cart/add_product_cart','', TRUE); ?>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                        ...
                        <div class="box-footer">
                            <button type="button" class="btn btn-primary submit_salebill pull-right"><?php echo $this->lang->line('save'); ?></button>
                        </div>
                    </form>
                </div>               
            </div>
...
<script type="text/javascript">
    function get_data_by_campuss(val){
        //alert();
        var url ='<?php echo site_url('admin/receiveitem/newpurchase/'); ?>'+val;          
        if(url){
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on('click', '.submit_salebill', function (e) {
            var itemsel = $('.itemsel').val();
            if(itemsel == ''){
                errorMsg('Please Select Before Product');
            }else{
                var $this = $(this);
                var formData = new FormData($('#submit_salebill')[0]);
                // var files = $this[0].files;
                // console.log(formData.append('file', $('#documents')[0].files[0]));
                    $this.button('loading');
                setTimeout(function () {
                    $.ajax({
                        url: '<?php echo site_url("admin/receiveitem/save_receiveitem") ?>',
                        type: 'POST',
                        data: formData,
                        contentType: false,
                        processData: false,
...
<script lang="javascript">
    $(document).ready(function() {
        ...
        //     row += '<select class="form-control js-example-basic-single" style="width: 100%">';
...
<script type="text/javascript">
    ...
    function pur_product_id(str) {
        ...
        //loader();
        $.ajax({
            url: '<?php echo site_url("admin/receiveitem/add_to_cart") ?>',
            type: "POST",
            data: {
                item_id : itemID,
                rowid: rowid,
            ...
            success: function(response) {
                // if(response == false){
                //     alert('Stock Qty Not Available Please Add Stock Qty!');
                // }
                //location.reload(); // then reload the page.(3)
                $.get(link + "admin/receiveitem/show_cart/", function (cart) {
                    //alert(cart);
                    $('#cart_view').html(cart);
                    $('.js-example-basic-single').select2();
                    //$('select').select2();
    ...
    function pur_updateItem(str) {
        ...
        var o_val = str.value;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/receiveitem/update_cart_item';
        //var csrftoken   = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_removeItem(str) {
        var rowid = str.id;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/receiveitem/remove_item';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_order_discount(str) {
        ...
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/receiveitem/order_discount';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_tax(str) {
        var tax =  str.value;
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/receiveitem/order_tax';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function pur_shipping(str) {
        ...
        var link = '<?php echo base_url() ?>';
        var postUrl     = link + 'admin/receiveitem/order_shipping';
        //var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function get_vendor(str) {
        ...
        var postUrl     = getBaseURL() + 'admin/receiveitem/select_vendor_by_id';
        var csrftoken = getCookie('csrf_cookie_name');
        //loader();
        $.ajax({
            url: postUrl,
    ...
    function getPurchaseProduct(str){
        ...
        if (req) {
            var link = '<?php echo base_url() ?>';
            $.post(link + "admin/receiveitem/add_purchase_product", {
                    purchase_product_id: purchaseProductId,
                    ajax: '1'
                },
                function (data) {
                    if (data == 'true') {
                        $.get(link + "admin/receiveitem/show_purchase", function (cart) {
