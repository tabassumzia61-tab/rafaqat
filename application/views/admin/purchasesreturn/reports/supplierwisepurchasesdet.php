<div class="content-wrapper">
    ...
    <section class="content">
        <?php $this->load->view('admin/purchases/reports/_purchasesreports'); ?>
        <div class="row">
            <div class="col-md-12">
                <div class="box removeboxmius">
                    ...
                    <form action="<?php echo site_url('admin/purchases/supplierwisepurchasesdet/'.$brc_id) ?>"  method="post" accept-charset="utf-8">
                        <div class="box-body">
                            <?php echo $this->customlib->getCSRF(); ?>
                            <div class="row">
                                <?php  
                                if ($this->rbac->hasPrivilege('supplier_wise_purchases_details', 'can_branch')) { ?>
                                    <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select autofocus="" id="supplier_id" name="supplier_id" class="form-control selectval">
                                            <option value=""><?php echo $this->lang->line('select'); ?></option>
                                            <?php foreach ($supplierlist as $sup_val) { ?>
                                                    <option value="<?php echo $sup_val['id'] ?>" <?php if (set_value('supplier_id') == $sup_val['id']) echo "selected=selected"; ?>><?php echo $sup_val['code'].' - '.$sup_val['name']; ?></option>
                                            <?php } ?>
                                        </select>
                    ...
                    <div class="row">
                        <div class="">
                            <div class="box-header ptbnull">
                                <h3 class="box-title titlefix"><i class="fa fa-list"></i> <?php echo $this->lang->line('supplier').' '.$this->lang->line('wise').' '.$this->lang->line('purchases').' '.$this->lang->line('details'); ?></h3>
                            </div>                              
                            <div class="box-body">
                                    ...
                                    <table class="table table-striped table-bordered table-hover example">
                                        ...
                                        <tbody id="tbldetail">
                                            ...
                                            if(!empty($purchaseslist)){
                                                foreach($purchaseslist as $purkey => $purval){ ?>
                                                <tr>
                                                    ...
                                                    <!--<td></td>-->
                                                    <td><?php echo $purval['supplier']; ?></td>
                                                    <td><?php echo $purval['note']; ?></td>
                                                    <td class="text text-right dr"><?php if(!empty($purval['dr'])){ echo (float)$purval['dr']; } ?></td>
                                                    <td class="text text-right cr"><?php //if(!empty($purval['cr'])){ echo (float)$purval['cr']; } ?></td>
...
<script type="text/javascript">
    function getBranchByID(val){
        //alert();
        var url ='<?php echo site_url('admin/purchases/supplierwisepurchasesdet/'); ?>'+val;          
        if(url){
