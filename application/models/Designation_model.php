...
class Designation_model extends MY_model {
    ...
    public function get($id = null) {
        $this->db->select('designation.*')->from('designation');
        if ($id != null) {
            $this->db->where('designation.id', $id);
        } else {
            $this->db->order_by('designation.id');
        }
        $query = $this->db->get();
        if ($id != null) {
    ...
    function check_designation_exists($name, $id) {
        if ($id != 0) {
            ...
            $query = $this->db->where($data)->get('designation');
            if ($query->num_rows() > 0) {
                return TRUE;
            } else {
                return FALSE;
    ...
    function addDesignation($data) {
        ...
        if (isset($data["id"])) {
            ...
            $this->db->where("id", $data["id"])->update("designation", $data);
