...
class Staff_model extends MY_Model
{
    ...
    public function get($id = null){        
        $this->db->select('staff.*,languages.language,languages.is_rtl,roles.name as user_type,roles.id as role_id')->from('staff')->join("roles", "staff.role_id = roles.id", "left")->join("languages", "languages.id = staff.lang_id", "left");
        
        if ($this->session->has_userdata('admin')) {
            if($this->staffrole->id != 1){
    ...
    public function getStaffByBrach($brach_id){
        $this->db->select('staff.*,languages.language,languages.is_rtl,roles.name as user_type,roles.id as role_id')->from('staff')->join("roles", "staff.role_id = roles.id", "left")->join("languages", "languages.id = staff.lang_id", "left");
    ...
    public function getrat()
    {
        ...
        $this->db->select('staff.id,staff.employee_id,CONCAT_WS(" ",staff.name,staff.surname,"(",staff.employee_id,")") as name,roles.name as user_type,roles.id as role_id,staff_rating.rate,staff_rating.status,staff_rating.comment,staff_rating.id as rate_id')->from('staff')->join("roles", "staff.role_id = roles.id", "left")->join("staff_rating", "staff_rating.staff_id = staff.id", "inner");
        $this->db->where('staff.is_active', 1);
        $this->db->where_not_in('roles.id', 1);
    ...
    {
        $this->db->select('staff_rating.rate,staff_rating.comment,staff_rating.role')->from('staff_rating')->join("staff", "staff_rating.staff_id = staff.id", "inner");
        $this->db->where('staff.is_active', 1);
    ...
    public function getpaybyID($id){
        $this->db->select('staff_pay.*,accountshead.name');
        $this->db->from('staff_pay');
        $this->db->join("accountshead", "accountshead.id = staff_pay.type_id");
        $this->db->where("staff_pay.staff_id", $id);
        $this->db->order_by('staff_pay.type_id', 'asc');
        $query = $this->db->get();
        return $query->result_array();
    }
    
    public function getpaydedbyID($id){
        $this->db->select('staff_pay_ded.*,accountshead.name');
        $this->db->from('staff_pay_ded');
        $this->db->join("accountshead", "accountshead.id = staff_pay_ded.type_id");
        $this->db->where("staff_pay_ded.staff_id", $id);
        $this->db->order_by('staff_pay_ded.type_id', 'asc');
        $query = $this->db->get();
        return $query->result_array();
    }
    
    public function getAlljv($brc_id){
        $this->db->select("staff.id,staff.employee_id as code,staff.name");
        $this->db->from('staff');
        $this->db->where('staff.barch_id', $brc_id);
        $this->db->where('staff.is_active',1);
        $this->db->order_by('staff.id');
        $query = $this->db->get();
        return $query->result_array();
    ...
    public function getAll($id = null, $is_active = null)
    {
        ...
        $this->db->select("staff.*,designation.designation,department.department_name as department, roles.id as role_id, roles.name as role");
        $this->db->from('staff');
        $this->db->join('designation', "designation.id = staff.designation", "left");
        $this->db->join('roles', "roles.id = staff.role_id", "left");
    ...
    public function batchInsert($data, $leave_array = array(), $data_setting = array()){
        $this->db->trans_start();
    ...
    public function assign_pay_staff($data_pay) {
        if (isset($data_pay['id'])) {
            $this->db->where('id', $data_pay['id']);
            $this->db->update('staff_assign_pay', $data_pay);
        } else {
            $this->db->insert('staff_assign_pay', $data_pay);
        }
    }

    public function remove_pay_assign($id) {
        $this->db->where('staff_id', $id);
        $this->db->delete('staff_assign_pay');
    }
    ...
    public function getStaffRole($id = null)
    {
        ...
        if ($id != null) {
            ...
            if ($this->superadmin_visible == 'disabled' && $this->staffrole->id != 1) {
                $this->db->where("roles.id !=", 1);             
            }
            $this->db->order_by('id'); 
        }
    ...
    public function getEmployee($role, $active = 1, $brach_id = null){
        $this->db->select("staff.*,branch.name as branch_name,designation.designation,department.department_name as department,roles.name as user_type,roles.id as role_id")->from('staff');
        $this->db->join('branch', "branch.id = staff.barch_id", "left");
        $this->db->join('designation', "designation.id = staff.designation", "left");
        $this->db->join('roles', "roles.id = staff.role_id", "left");
        $this->db->join('department', "department.id = staff.department", "left");    
        $this->db->where("staff.is_active", $active);  
        if($role != ""){
            $this->db->where("staff.role_id", $role);
        }
        if($brach_id != ""){
            $this->db->where("staff.barch_id", $brach_id);
        }   
    ...
    public function getEmployeeByRoleID($role, $active = 1)
    {
        ...
        $query = $this->db->select("staff.*,designation.designation,department.department_name as department, roles.id as role_id, roles.name as role")->join('designation', "designation.id = staff.designation", "left")->join('roles', "roles.id = staff.role_id", "left")->join('department', "department.id = staff.department", "left")->where("staff.is_active", $active)->where("roles.id", $role)->get("staff");
    ...
    public function getStaffDesignation()
    {
        ...
        $query = $this->db->select('*')->where("is_active", "yes")->get("designation");
        return $query->result_array();
    }
    ...
    public function getProfile($id)
    {
        ...
        $this->db->select('staff.*,designation.designation as designation,roles.id as role_id, department.department_name as department,roles.name as user_type');
        $this->db->join("designation", "designation.id = staff.designation", "left");
        $this->db->join("department", "department.id = staff.department", "left");
    ...
    public function searchFullText($searchterm,$selected_category, $active){
        $this->db->select("staff.*,branch.name as branch_name,designation.designation,department.department_name as department,roles.name as user_type")->from('staff');
        $this->db->join('branch', "branch.id = staff.barch_id");
        $this->db->join('designation', "designation.id = staff.designation", "left");
        $this->db->join('roles', "roles.id = staff.role_id", "left");
        $this->db->join('department', "department.id = staff.department", "left");
        $this->db->where("staff.is_active", $active);
        if($selected_category === 'staff_id') {
            $this->db->where('staff.employee_id', $searchterm);
        }elseif($selected_category === 'name') {
            $this->db->group_start();
            $this->db->like('staff.name', $searchterm);
            $this->db->or_like('staff.surname', $searchterm);
            $this->db->group_end();
        }elseif($selected_category === 'role') {
            $this->db->where("roles.name", $searchterm);
        }
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function checkLogin($data)
    {
        ...
        if ($record) {
            ...
            if ($pass_verify) {
                $roles = $this->staffroles_model->getStaffRoles($record->role_id);
                $record->roles = array($roles->name => $roles->id);
                return $record;
            }
        }
        return false;
    ...
    public function getStaffByRoleByBranch($id,$brach_id){
        $this->db->select('staff.*,designation.designation as designation,roles.id as role_id, department.department_name as department,roles.name as user_type');
        $this->db->join("designation", "designation.id = staff.designation", "left");
        $this->db->join("department", "department.id = staff.department", "left");
        $this->db->join("roles", "staff.role_id = roles.id", "left");
    ...
    {       
        ...
        $query = "SELECT `staff`.*, `designation`.`designation` as `designation`, `department`.`department_name` as `department`,`roles`.`name` as user_type " . $field_var . ",GROUP_CONCAT(leave_type_id,'@',alloted_leave) as leaves  FROM `staff` " . $join_array . " LEFT JOIN `designation` ON `designation`.`id` = `staff`.`designation` LEFT JOIN `roles` ON `staff`.`role_id` = `roles`.`id` LEFT JOIN `department` ON `department`.`id` = `staff`.`department` left join staff_leave_details ON staff_leave_details.staff_id=staff.id WHERE 1  " . $condition .    $rolescondition . " group by staff.id";
    ...
    {
        $this->db->select('staff.*,languages.language,roles.name as user_type,roles.id as role_id')->from('staff')->join("roles", "staff.role_id = roles.id", "left")->join("languages", "languages.id = staff.lang_id", "left");
        $this->db->where('staff.id', $id); 
        $query = $this->db->get();
        return $query->result_array();
    ...
    public function removepay($id) {
        $this->db->where('id', $id);
        $this->db->delete('staff_pay');
    }
    
    public function removepayded($id) {
        $this->db->where('id', $id);
        $this->db->delete('staff_pay_ded');
    }
    
    public function assignpaystaff($data_pay) {
        if (isset($data_pay['id'])) {
            $this->db->where('id', $data_pay['id']);
            $this->db->update('staff_pay', $data_pay);
        } else {
            $this->db->insert('staff_pay', $data_pay);
        }
    }
    
    public function assignpaydedstaff($data_pay) {
        if (isset($data_pay['id'])) {
            $this->db->where('id', $data_pay['id']);
            $this->db->update('staff_pay_ded', $data_pay);
        } else {
            $this->db->insert('staff_pay_ded', $data_pay);
        }
    }
}
